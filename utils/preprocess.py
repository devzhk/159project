...
def transform_to_svd_components(data,
    ...
    if svd_computer is None:
        ...
        svd_data = svd_computer.transform(centered_data)
        explained_variances = np.var(
            svd_data, axis=0) / np.var(centered_data, axis=0).sum()
...
def transform_svd_to_keypoints(data, svd_computer, mean, stack_agents=False,
    ...
    mouse_kps = [svd_computer.inverse_transform(
        comps) + mean for comps in svd_components]
...
def preprocess(raw_data, label=False):
    full_dataset = []
