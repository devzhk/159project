...
from .loss import compute_label_loss, compute_decoding_loss, compute_contrastive_loss
...
class TREBA_model(BaseSequentialModel):
    ...
    def model_params(self):
        params = list(self.enc_birnn.parameters()) + list(self.enc_mean.parameters()) + list(
            ...
            if self.loss_params['decoding_loss_weight'] > 0:
                ...
                for lf_idx, lf_name in enumerate(labels_dict):
                    ...
                    self.log.losses["decoded_" + lf_name] = compute_decoding_loss(posterior.mean,
                                                                                  lf_labels,
                                                                                  self.label_decoder_fc_decoding[
                                                                                      lf_idx],
                                                                                  lf.categorical,
                                                                                  loss_weight=self.loss_params[
                                                                                      'decoding_loss_weight'])
            ...
            if 'augmentations' in self.config.keys():
                for aug in self.config['augmentations']:
                    ...
                    for lf_idx, lf_name in enumerate(labels_dict):
                        ...
                        lf_labels = labels_dict[lf_name]
                        if self.loss_params['contrastive_loss_weight'] > 0:
                            self.log.losses[aug.name + "_contrastive_" + lf_name] = compute_contrastive_loss(
                                posterior.mean, aug_posterior.mean,
                        ...
                        if self.loss_params['decoding_loss_weight'] > 0:
                            self.log.losses[aug.name + "_decoded_" + lf_name] = compute_decoding_loss(
                                aug_posterior.mean,
                    ...
                    if len(labels_dict) == 0 and self.loss_params['contrastive_loss_weight'] > 0:
                        ...
                        self.log.losses[aug.name + '_contrastive'] = compute_contrastive_loss(posterior.mean,
                                                                                              aug_posterior.mean,
            ...
            if (('augmentations' not in self.config.keys() or len(self.config['augmentations']) == 0)
                ...
                if len(labels_dict) == 0:
                    ...
                    for lf_idx, lf_name in enumerate(labels_dict):
                        ...
                        self.log.losses["contrastive_" + lf_name] = compute_contrastive_loss(posterior.mean,
                                                                                             posterior.mean,
                                                                                             lf_idx, labels=lf_labels,
                                                                                             temperature=
                                                                                             self.loss_params[
                                                                                                 'contrastive_temperature'],
                                                                                             base_temperature=
