...
def train(model, data_loader, train_config, logdir, device):
    ...
    for epoch in range(train_config['start_epoch'], train_config['num_epochs']):
        ...
        if epoch % train_config['num_epochs_til_val'] == 0:
            with open(f'{logdir}/{train_config["project_name"]}/val_losses.txt', 'a') as myFile:
        ...
        with open(f'{logdir}/{train_config["project_name"]}/train_losses.txt', 'a') as myFile:
